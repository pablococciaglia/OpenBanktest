...
export const CreatePasswordScreen = () => {
	...
	const { checkpoints, setcheckpoints, setActiveStep, setapiresponse } =
	...
	const validate = Yup.object({
		...
		pista: Yup.string().max(
			255,
			'La pista no puede superar los 255 caracteres'
		),
	});

	return (
		<>
			<img src={triangulo} alt='triangulo' className='screen2__triangulo' />{' '}
			{/* Imagen triangular que señala en que paso del step-progressbar nos encontramos */}
			<div className='base__container'>
				{isLoading && <Loading />}{' '}
				{/* cuando isLoading está en true se mostrará el componente Loading */}
						...
						{(formik) => (
							<Form>
								<div className='screen2__rulesbox'>
									...
									{/* se pasan por props los valores del formulario en tiempo real para editar la checklist de la contaseña */}
								</div>
								...
								<div className='screen2__pistabox'>
									<h3>
										...
										<Tooltip
											...
											arrow
										>
											<i className='bi bi-info-circle screen2__infoicon'></i>
										</Tooltip>
									</h3>
									<TextField
										label='Crea tu pista para recordar tu contraseña maestra (opcional)'
										maxLength='255'
										name='pista'
										type='text'
										placeholder='Introduce tu pista'
									/>
									<div className='screen2__contadorbox'>
										<h3 className='screen2__contador'>
											{formik.values.pista.length}/255
										</h3>
									</div>
								</div>
